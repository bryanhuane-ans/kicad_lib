...
def main():
    ...
    # === 1️⃣ Copiar el netlist (.xml) a la carpeta /fab/netlist ===
    input_file = os.path.abspath(args.netlist)
    input_dir = os.path.abspath(os.path.dirname(input_file))

    netlist_output_dir = os.path.abspath(os.path.join(input_dir, "..", "fab", "netlist"))

    if not os.path.exists(netlist_output_dir):
        os.makedirs(netlist_output_dir)
        debug.message(f"Creating netlist export directory: '{netlist_output_dir}'")

    netlist_copy = os.path.join(netlist_output_dir, os.path.basename(input_file))
    try:
        shutil.copy(input_file, netlist_copy)
        debug.message(f"Copied netlist to: '{netlist_copy}'")
        try:
            os.remove(input_file)
            debug.message(f"Deleted original netlist from project: '{input_file}'")
        except Exception as e:
            debug.warning(f"Could not delete original netlist: {e}")
    except Exception as e:
        debug.warning(f"Could not copy netlist: {e}")

    # Trabajar a partir de la copia
    input_file = netlist_copy

    # === 2️⃣ Carpeta de salida para el BOM ===
    default_output_dir = os.path.abspath(os.path.join(input_dir, "..","..", "fab", "bom"))
    ...
    if args.subdirectory is not None:
        ...
        if not os.path.exists(output_dir):
            ...
            debug.message(f"Creating subdirectory: '{output_dir}'")
    else:
        output_dir = default_output_dir
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
            debug.message(f"Creating default output directory: '{output_dir}'")

    debug.message(f"Output directory: '{output_dir}'")
    ...
    # --- 3️⃣ Configuración del bom.ini ---
    base_ini = os.path.abspath(os.path.join(input_dir, "..","..", "lib", "tools", "KiBoM", "bom_ans.ini"))
    config_file = base_ini

    # Si el usuario pasa uno personalizado, lo sobreescribe
    if args.cfg:
